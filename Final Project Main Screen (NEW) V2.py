...
def datatable(name,value,netchange,time):
    nvdatable = stockclass.Stocktable(win,"NAME", "VALUE(USD)", "", "NETCHANGE", "", "", "TIME")
...
while(game): #Main while loop to keep the game running 
    ...
    while instockwatch: #Main while loop for the stockwatch section of the game
        ...
        newupdate = stockclass.Button(win,(900,0,270,70),BLACK,"UPDATE",RED)
        newupdate.createbutton()
        ##--------------------------------------------------------------------------------------##
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if clicknewupdate == True:
                    print("..Updating Stock Prices..")
                    print("Please Wait....")
                    update_realtime_prices(m)
                    print("Stock Prices Succesfully Updated!")
        ...
        for i in range(len(m.data)):
            if chosenstock[i] == True: #If the button is clicked
                ...
                #Change length of graph line to //60
